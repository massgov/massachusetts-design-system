...
function getCssOutputPath(inputFile) {
  const parsedPath = path.parse(inputFile);

  return path.join(parsedPath.dir, `${parsedPath.name}.css`);
}

function getGeneratedCssBanner(inputFile) {
  return `/* This file is generated by components/scripts/build.js. Edit ${path.basename(inputFile)} instead. */`;
}
...
async function rewriteHtmlCdnReferences(dir) {
  const htmlFiles = await getFilesByExtension(dir, '.html');
...
async function compileComponentSassFiles() {
  const scssFiles = await getFilesByExtension(srcDir, '.scss');

  await Promise.all(
    scssFiles.map(async (scssFile) => {
      const cssFile = getCssOutputPath(scssFile);

      await compileSass(scssFile, cssFile);

      const compiledCss = await fs.readFile(cssFile, 'utf8');
      await fs.writeFile(
        cssFile,
        `${getGeneratedCssBanner(scssFile)}\n${compiledCss}`,
        'utf8'
      );
    })
  );
...
async function build() {
  await cleanDist();
  await compileComponentSassFiles();
