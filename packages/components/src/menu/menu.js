let popupMenu = {
   ...
   * Prevents duplicate setup for triggers and menus that have already been wired.
   *
   * @param {ParentNode} [root=document] - The DOM subtree containing popup menu triggers and menus to initialize.
   */
  init: function (root = document) {
    let popupMenuTriggers = root.querySelectorAll(".mds-popup-menu-trigger");
    let menuItemSelector = "[role='menuitem']";
    let getAllMenus = function () {
      return document.querySelectorAll(".mds-popup-menu");
    };
    ...
    popupMenuTriggers.forEach((trigger) => {
      if (trigger.dataset.mdsPopupMenuInitialized === "true") {
        return;
      }
      ...
      trigger.dataset.mdsPopupMenuInitialized = "true";
    ...
    getAllMenus().forEach((menu) => {
      if (menu.dataset.mdsPopupMenuInitialized === "true") {
        return;
      }

      menu.dataset.mdsPopupMenuInitialized = "true";
